...
def is_valid_svg(path: Path) -> bool:
    try:
        if not path.is_file() or path.stat().st_size == 0:
            return False
        head = path.read_bytes()[:SVG_HEAD_BYTES].decode("utf-8", errors="ignore").lower()
    except OSError:
        return False
    if "<svg" not in head:
        return False
...
def process(entries: Iterable[LogoEntry], logo_dir: Path, force: bool) -> dict[str, int]:
...
def main(argv: list[str]) -> int:
    ...
    parser.add_argument("--force", action="store_true", help="re-download even if local SVG is valid")
