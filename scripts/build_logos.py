...
def badge_width(label: str, logo_mode: str = "icon") -> int:
    ...
    return int(round(LEFT_PAD + LOGO_SIZE + LOGO_TEXT_GAP + len(label) * CHAR_W + RIGHT_PAD))
...
def process(
    ...
    for entry in entries:
        ...
        if not is_valid_svg(logo_path):
            ...
            print(status_line("FAILED", entry.name, f"missing or invalid logo at {logo_path}"))
            outcome.failed += 1
            continue
